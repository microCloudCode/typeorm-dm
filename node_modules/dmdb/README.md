...
### dmdb v1.0.48286(2026-03-23)
- *内部Buffer性能优化
- *`Connection.executeMany()`批量执行接口的绑出参数`Results.outBinds`的格式从以行分组改为以列分组，与oracledb表现一致
- 修复了以绑定参数插入一定长度的字符串时的报错：[6067] 字符串截断
- 删除`ResultSet.getRowCount()`接口，因为仅向前结果集在遍历完成前无法提前获取总行数
- 连接池默认别名改为'default'，根据别名未获取到连接池时改为报错
- 一些兼容oracledb特性的调整
- 新增连接串参数: cmini、pstmtPoolSize、pstmtPoolValidTime。详情查看index.d.ts文件中ConnectionAttributes

### dmdb v1.0.46190(2026-01-20)
- 修复了以流方式读取带有emoji字符的大字段时的报错：[6057] 长度或偏移错误
